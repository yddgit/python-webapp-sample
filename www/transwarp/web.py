...
'''
# 流行的Web框架特点
# 1.Django：一站式开发框架，但不利于定制化
# 2.web.py：使用类而不是更简单的函数来处理URL，并且URL映射是单独配置的
# 3.Flask：使用@decorator的URL路由不错，但框架对于应用程序的代码入侵太强
# 4.bottle：缺少根据URL模式进行拦截的功能，不利于做权限检查

# 一个简单的URL框架应该允许以@decorator方式直接把URL映射到函数上
# 有没有@decorator不改变函数行为，也就是说API入侵性很小，可以直接测试函数而不需要启动Web服务器
@get('/')
def index():
...
# Web框架要支持URL拦截器，这样就可以根据URL做权限检查
# 拦截器接受一个next函数，决定调用next()继续处理请求还是直接返回
@interceptor('/manage/')
...
# 为了支持MVC，Web框架需要支持模板，但不限定使用哪一种模板
# 统一模板接口，函数可以返回dict并配合@view来渲染模板
@view('index.html')
@get('/')
def index():
    return dict(blogs=get_recent_blogs(), user=get_current_user())

# 如果需要从form表单或者URL的querystring获取用户输入的数据，要访问request对象
# 如果要设置特定的Content-Type、设置Cookie等，需要访问response对象
# request和response对象应该从一个唯一的ThreadLocal中获取
@get('/test')
def test():
...
# 最后，如果需要重定向或者返回一个HTTP错误码，最好的方法是直接抛出异常
raise seeother('/signin')
# 返回404错误
raise notfound()
...
# 全局ThreadLocal对象，用于存储request和response对象
ctx = threading.local()
...
class Jinja2TemplateEngine(TemplateEngine):
    u'''
    缺省使用jinja2。Render using jinja2 template engine.
